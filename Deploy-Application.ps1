...
Try {
	...
	[string]$appVersion = '4.7.4056.0'
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.1'
	[string]$appScriptDate = '10/25/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'zotero' -CheckDiskSpace -PersistPrompt
		...
		$exitCode = Execute-Process -Path "$dirFiles\anyconnect-win-4.7.04056-core-vpn-webdeploy-k9.exe" -Parameters "/quiet" -WindowStyle "Hidden" -PassThru
		...
		Copy-Item -Path "$dirFiles\Import-Preferences.bat" -Destination "${env:ProgramFiles(x86)}\Cisco"
		Copy-Item -Path "$dirFiles\preferences.xml" -Destination "${env:ProgramFiles(x86)}\Cisco"

		#Active Setup to copy user preferences
		...
		Set-ItemProperty -Path $ActiveSetupRegPath -Name 'StubPath' -Value "${env:ProgramFiles(x86)}\Cisco\Import-Preferences.bat" -Force
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		If (Test-Path "${env:ProgramFiles(x86)}\Cisco\Cisco AnyConnect Secure Mobility Client\Uninstall.exe") {
			##$exitCode = Execute-Process -Path "$dirFiles\anyconnect-win-4.7.04056-core-vpn-webdeploy-k9.exe" -Parameters "/uninstall" -WindowStyle "Hidden" -PassThru
			$exitCode = Execute-Process -Path "${env:ProgramFiles(x86)}\Cisco\Cisco AnyConnect Secure Mobility Client\Uninstall.exe" -Parameters "-remove -silent" -WindowStyle "Hidden" -PassThru
			If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
			...
			While($uninstallerExists){
				...
				Else {
					...
					Write-Log -Message "Uninstall.exe no longer detected. Continuing..." -Source 'Pre-Installation' -LogType 'CMTrace'
				}
			}
		}
		Else {
			Write-Log -Message "Uninstaller not detected." -Source 'Uninstallation' -LogType 'CMTrace'
		}
		...
		## Clean up the Active Setup stuff
		If(Test-Path -Path "HKLM:\Software\Microsoft\Active Setup\Installed Components\SetupCiscoAnyConnectPrefs"){
			Remove-Item -Path "HKLM:\Software\Microsoft\Active Setup\Installed Components\SetupCiscoAnyConnectPrefs" -Force
		}
		If (Test-Path "${env:ProgramFiles(x86)}\Cisco\Import-Preferences.bat") {
			Write-Log -Message "Deleting Import-Preferences.bat" -Source 'Pre-Installation' -LogType 'CMTrace'
			Remove-Item "${env:ProgramFiles(x86)}\Cisco\Import-Preferences.bat" -Force
		}
		Else {
			Write-Log -Message "Import-Preferences.bat not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		If (Test-Path "${env:ProgramFiles(x86)}\Cisco\preferences.xml") {
			Write-Log -Message "Deleting preferences.xml" -Source 'Pre-Installation' -LogType 'CMTrace'
			Remove-Item "${env:ProgramFiles(x86)}\Cisco\preferences.xml" -Force
		}
		Else {
			Write-Log -Message "preferences.xml not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
